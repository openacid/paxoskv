			...
			for b := a + 1; b < 3; b++ {
				...
				for i := 0; i < len(ca); i++ {
					for j := 0; j < len(cb); j++ {
						insta := ca[i]
						instb := cb[j]
						if insta.Deps[b] > int64(j) || instb.Deps[a] > int64(i) {
							// there is a relation
						} else {
							ta.Fail("no relation:", "%d-%d and %d-%d : seen: %v %v", a, i, b, j, insta.Deps, instb.Deps)
						}
					}
				}
			}
...

	{
		...
		for column := 0; column < 3; column++ {
			for lsn := 0; lsn < len(s.log.columns[column].Log); lsn++ {
				...
				v0 := i0.Val.str() + " " + fmt.Sprintf("%d,%d,%d", i0.Deps[0], i0.Deps[1], i0.Deps[2])
				v1 := i1.Val.str() + " " + fmt.Sprintf("%d,%d,%d", i1.Deps[0], i1.Deps[1], i1.Deps[2])
				v2 := i2.Val.str() + " " + fmt.Sprintf("%d,%d,%d", i2.Deps[0], i2.Deps[1], i2.Deps[2])
				fmt.Printf("s0 %d-%d: %s\n", column, lsn, v0)
				fmt.Printf("s1 %d-%d: %s\n", column, lsn, v1)
				fmt.Printf("s2 %d-%d: %s\n", column, lsn, v2)
