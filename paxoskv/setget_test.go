...
func Test_set_get(t *testing.T) {
	...
	for sIdx, s := range servers {
		...
		for column := 0; column < 3; column++ {
			...
			fmt.Println("column:", column, "next:c/a", col.nextCommit, col.nextApply)
...

	{
		...
		total := 0
		cnt := map[string]int{}
		for column, col := range s.columns {
			_ = column
...

	{
		...
		for column := 0; column < 3; column++ {
			for lsn := 0; lsn < len(s.columns[column].Log); lsn++ {
				i0 := servers[0].columns[column].Log[lsn]
				i1 := servers[1].columns[column].Log[lsn]
				i2 := servers[2].columns[column].Log[lsn]
...

	{
		...
		for _, v := range s.applySeq {
			fmt.Println(colLSNIndentedStr(v))
		}

		for i := range servers {
			if alive[i] {
				ta.Equal(s.applySeq, servers[i].applySeq, "server: %d", i)
			}
		}
	}

	{
		...
		for i := range servers {
			if alive[i] {
				ta.Equal(shot, servers[i].getSnapshot(), "server %d", i)
			}
		}
	...
	// for _, s := range servers {
	//     ta.EqualValues(nreq, s.nextApply)
