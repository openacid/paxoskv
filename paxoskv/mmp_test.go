...
func TestKVServer_hdlPrepare(t *testing.T) {
	...
	s.log.columns[0] = &Column{
		LastBal: NewBal(0, 0),
		Log:     []*Ins{nil, {InsId: NewInsId(0, 1, 0), Val: &Cmd{Key: "foo", Vi64: 10}, Seen: []int64{3, 0, 3}}},
	}

	{
		// prepare a nil instnace 0-0
		lsn := int64(0)
		inst := &Ins{InsId: NewInsId(0, lsn, 1), Val: &Cmd{}, Seen: []int64{1, 0, 5}}
...

	{
		...
		lsn := int64(1)
		inst := &Ins{InsId: NewInsId(0, lsn, 3), Val: &Cmd{}, Seen: []int64{1, 0, 5}}
...
func TestKVServer_hdlOps(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "vnil->nil",
			inst: &Ins{InsId: NewInsId(column, lsn, 0), Val: cmdV.Clone(), VBal: nil, Seen: []int64{0, 1, 5}},
			logs: []*Ins{},
		...
		},
		{
			...
			inst: &Ins{InsId: NewInsId(column, lsn, 0), Val: cmdV.Clone(), VBal: NewBal(1, 0), Seen: []int64{0, 1, 5}},
			logs: []*Ins{},
		...
		{
			name: "v0->w1",
			inst: &Ins{InsId: NewInsId(column, lsn, 0), Val: cmdV.Clone(), VBal: NewBal(0, 0), Seen: []int64{0, 1, 5}},
			logs: []*Ins{
				{InsId: NewInsId(column, lsn, 1), Val: cmdW.Clone(), VBal: NewBal(2, 1), Seen: []int64{3, 0, 3}},
			},
		...
		{
			name: "v2->w1",
			inst: &Ins{InsId: NewInsId(column, lsn, 0), Val: cmdV.Clone(), VBal: NewBal(2, 0), Seen: []int64{0, 1, 5}},
			logs: []*Ins{
				{InsId: NewInsId(column, lsn, 1), Val: cmdW.Clone(), VBal: NewBal(1, 1), Seen: []int64{3, 0, 3}},
			},
			wantInst: "<1-0-0: <V=0> vbal:10,2 c:true seen:0,1,5>",
			wantLogs: "<1-0-0: <V=0> vbal:10,2 c:true seen:0,1,5>",
		},
		{
			...
			inst: &Ins{InsId: NewInsId(column, lsn, 0), Val: cmdV.Clone(), VBal: nil, Seen: []int64{0, 1, 5}},
			logs: []*Ins{
				{InsId: NewInsId(column, lsn, 1), Val: cmdW.Clone(), VBal: nil, Seen: []int64{3, 0, 3}},
			},
