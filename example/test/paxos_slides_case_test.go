...
func TestCase1SingleProposer(t *testing.T) {
	...
	var val int64 = 10
	paxosId := &paxoskv.PaxosInstanceId{
		Key: "i",
		Ver: 0,
	}
	...
	px := paxoskv.Proposer{
		Id:  paxosId,
		Bal: &paxoskv.BallotNum{N: 0, ProposerId: pidx},
	}
...
func TestCase2DoubleProposer(t *testing.T) {
	...
	px := paxoskv.Proposer{
		Id:  paxosId,
		Bal: &paxoskv.BallotNum{N: 1, ProposerId: pidx},
	}
	latestVal, higherBal, err := px.Phase1([]int64{0, 1}, quorum)
	...
	py := paxoskv.Proposer{
		Id:  paxosId,
		Bal: &paxoskv.BallotNum{N: 2, ProposerId: pidy},
	}
	...
	px.Val = &paxoskv.Value{Vi64: 100}
	higherBal, err = px.Phase2([]int64{0, 1}, quorum)
	ta.Equalf(err, paxoskv.NotEnoughQuorum, "Proposer X should fail in phase-2")
	...
	px.Val = nil
	px.Bal = &paxoskv.BallotNum{N: 3, ProposerId: pidx}
	latestVal, higherBal, err = px.Phase1([]int64{0, 1}, quorum)
	ta.Nil(err, "constituted a quorum")
